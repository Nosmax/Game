...
const MouseGame: React.FC<MouseGameProps> = ({ difficulty }) => {
  ...
  const handleHoleClick = (index: number) => {
    if (mice.includes(index)) {
      const numberOfMice = mice.filter(
        (mouseIndex) => mouseIndex === index
      ).length;
      console.log(
        `Souris(s) trouvée(s) au trou ${index} ! Score +${numberOfMice}`
      );
      setScore((prevScore) => prevScore + numberOfMice);
      setMice((prevMice) =>
        prevMice.filter((mouseIndex) => mouseIndex !== index)
      );
    } else {
